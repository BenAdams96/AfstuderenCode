...
def main(custom_foldername = 'custom_dataframes', list_of_features = [5,1,2,106,21,0,20,8,101,31,76,75,10,102,74,3,19,30,100,9,54,32,13,22,67]):
    custom_path = public_variables.dataframes_master_ / custom_foldername
    custom_path.mkdir(parents=True, exist_ok=True)

    list_of_features = ['mol_id','PKI','conformations (ns)'] + list(map(str, list_of_features))

    timeinterval = [1,0.5,0.2,0.1,0.05,0.02]
    big_df = pd.read_csv(public_variables.dataframes_master_ / 'conformations_1000.csv', usecols=list_of_features)

    dfs_in_dict = create_dfs_dic(big_df, timeinterval = 1)
    save_dataframes(dfs_in_dict,custom_path)

    for t in timeinterval:
        print(t)
        reduced_dataframe = reduce_conformations(big_df, interval=t)
        reduced_dataframe.to_csv(custom_path / f'conformations_{int(10/t)}.csv', index=False)
        print('done')
    return
