...
def plot_feature_distributions(df, max_columns=5):
    num_features = df.shape[1]
    nrows = int(np.ceil(num_features / max_columns))
    fig, axes = plt.subplots(nrows, max_columns, figsize=(5 * max_columns, 4 * nrows))
    axes = axes.flatten()

    for i, col in enumerate(df.columns):
        ax = axes[i]
        data = df[col].dropna()  # Drop NaN values for clearer plots

        # Determine if the distribution has a large range, and if so, use log scaling
        if data.max() - data.min() > 1000:
            ax.hist(data, bins=50, log=True, color='skyblue')
            #ax.set_yscale('log')
        else:
...
# def plot_overlay_histograms(df, target_col='PKI', threshold=7.5, bins=30, max_columns=5):
#     features = [col for col in df.columns if col != target_col]  # Exclude target column
#     nrows = int(np.ceil(len(features) / max_columns))
#     fig, axes = plt.subplots(nrows, max_columns, figsize=(5 * max_columns, 4 * nrows))
#     axes = axes.flatten()
...
#         data_high = df[df[target_col] >= threshold][feature].dropna()
#         data_low = df[df[target_col] < threshold][feature].dropna()

#         # Overlay histograms with high data on top
#         ax.hist(data_high, bins=bins, color='salmon', edgecolor='black', alpha=0.6, label=f'{target_col} >= {threshold}')
#         ax.hist(data_low, bins=bins, color='skyblue', edgecolor='black', alpha=0.6, label=f'{target_col} < {threshold}')
...
# def plot_three_overlay_histograms(df, target_col='PKI', bins=30, max_columns=5):
#     features = [col for col in df.columns if col != target_col]  # Exclude target column
#     nrows = int(np.ceil(len(features) / max_columns))
#     fig, axes = plt.subplots(nrows, max_columns, figsize=(5 * max_columns, 4 * nrows))
#     axes = axes.flatten()
...
#         # Split data based on the threshold
#         data_low = df[df[target_col] < low][feature].dropna()
#         data_mid = df[(df[target_col] >= low) & (df[target_col] <= high)][feature].dropna()
#         data_high = df[df[target_col] > high][feature].dropna()

#         # Overlay histograms
#         ax.hist(data_high, bins=bins, color='lightgreen', edgecolor='black', alpha=0.6, label=f'PKI > {high}')
        
#         ax.hist(data_mid, bins=bins, color='salmon', edgecolor='black', alpha=0.6, label=f'{low} ≤ PKI ≤ {high}')
#         ax.hist(data_low, bins=bins, color='lightblue', edgecolor='black', alpha=0.6, label=f'PKI < {low}')
...
def plot_three_overlay_histograms_with_density(df, target_col='PKI', bins=30, max_columns=5):
    features = [col for col in df.columns if col != target_col]  # Exclude target column
    nrows = int(np.ceil(len(features) / max_columns))
    fig, axes = plt.subplots(nrows, max_columns, figsize=(5 * max_columns, 4 * nrows))
    axes = axes.flatten()
    ...
    for i, feature in enumerate(features):
        ...
        # Overlay histograms with density normalization
        ax.hist(data_high, bins=bins, color='lightgreen', edgecolor='black', alpha=0.6, 
                label=f'PKI > {high}', density=True)
        ax.hist(data_mid, bins=bins, color='salmon', edgecolor='black', alpha=0.6, 
                label=f'{low} ≤ PKI ≤ {high}', density=True)
        ax.hist(data_low, bins=bins, color='lightblue', edgecolor='black', alpha=0.6, 
                label=f'PKI < {low}', density=True)
...
def main(dfs_descriptors_path = public_variables.dfs_descriptors_only_path_):
    big_df = pd.read_csv(public_variables.dfs_descriptors_only_path_ / '1ns.csv')
    big_df = pd.read_csv(public_variables.dfs_MD_only_path_ / 'conformations_10.csv')
    big_df = pd.read_csv(public_variables.dfs_reduced_path_ / '1ns.csv')
    plot_three_overlay_histograms(big_df)
    # plot_three_overlay_histograms_with_density(big_df)
...
if __name__ == "__main__":
    main(dfs_descriptors_path = public_variables.dfs_descriptors_only_path_)
