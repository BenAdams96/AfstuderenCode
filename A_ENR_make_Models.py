...
from A_ENR_class import ElasticNetRegressor
...
def Kfold_Cross_Validation_incl_grouped(dfs_in_dic, hyperparameter_grid, kfold_, scoring_):
    ...
    for name, df in dfs_in_dic.items():
        ...
        elastic_net_model = ElasticNetRegressor(alpha=1.0, l1_ratio=0.5)  # Default values, customize as needed

        # Drop columns not used in features
        X_train = df.drop(columns=['mol_id', 'PKI', 'conformations (ns)'], axis=1, errors='ignore')

        # Perform grid search with the SVM model
        grid_search = elastic_net_model.hyperparameter_tuning(X_train, targets, hyperparameter_grid, cv=custom_splits, scoring_=scoring_)

        # Collect results
        df_results = pd.DataFrame(grid_search.cv_results_)
        result = df_results.loc[grid_search.best_index_, columns_[1:]]
        elastic_net_model.print_feature_stats(X_train)
...
def main(dfs_path = public_variables.dfs_descriptors_only_path_):  ###set as default, but can always change it to something else.
    ...
    Modelresults_path = dfs_path / 'ModelResults_ENR'
    ...
    hyperparameter_grid = public_variables.hyperparameter_grid_ENR #n_estimators, max_depth etc.
